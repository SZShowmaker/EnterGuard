...
- **第二阶段: 风险检测**, 高风险才弹窗, 低风险静默放行。三类检测, 任一命中即高风险:
  1. 聊天对象变化: 切换群/应用且在阈值窗口内 (默认阈值0=任何变化都弹, 可配置)
  2. 敏感词: 消息含敏感词(工资/密码/身份证/脏话等), 词表可在GUI编辑
  3. 静默时段: 非工作时间(如 22:00~08:00)内一律高风险, 支持跨午夜, 起止时间可配置
  - 首次发送默认放行(无基准), 但命中 2/3 仍弹
  - 会话名提取: 老版钉钉从标题切分(如 "群名 - 钉钉"); 新版钉钉Qt自绘读不到群名, 用会话列表"选中项索引"作指纹(如 `钉钉#0`), 虽无真名但能检测群切换
  - 已知限制: 飞书(Chromium内核)和微信(Qt自绘)的会话列表/输入框均未暴露给UIA, 仅 app级切换 + 静默时段 可用 (详见下表)
- 未来可扩展: 白名单等
...
拦截 Enter 后, 主线程会先做风险判断, 低风险静默放行, 高风险才弹窗。三类检测任一命中即视为高风险:
...
**首次发送**: 无基准, 默认低风险放行; 但若命中敏感词/静默时段仍弹。

**会话名提取策略** (优先级递减):
1. **标题切分**: 老版钉钉标题格式 `群名 - 钉钉`, 可切出真群名。
