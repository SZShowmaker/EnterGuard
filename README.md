...
- **第二阶段: 风险检测**, 高风险才弹窗, 低风险静默放行。五类检测, 任一命中即高风险:
  1. 聊天对象变化: 切换群/应用且在阈值窗口内 (默认阈值0=任何变化都弹, 可配置)
  2. 敏感词: 消息含敏感词(工资/密码/身份证/脏话等), 词表可在GUI编辑
  3. @提及: `@所有人`/`@all` 默认弹, 可配置为"任何@都弹"或"关闭", 可加自定义@触发词(如 @老板)
  4. 高危群: 群名命中关键词(客户/外部/全员/领导等)则**每次必弹**, 不受对象变化逻辑管
  5. 静默时段: 非工作时间(如 22:00~08:00)内一律高风险, 支持跨午夜, 起止时间可配置
  - 首次发送默认放行(无基准), 但命中 2/3/4/5 仍弹
...
- `at_trigger_mode`: `all`(默认, 仅@所有人/@all弹) | `any`(任何@都弹) | `off`(关闭). 配合 `at_extra_keywords` 用.
- `at_all_keywords`: 视为"@所有人"的关键词 (默认 `@所有人`/`@all`/`@全体`).
- `at_extra_keywords`: 用户自定义的@触发词 (如 `@老板`), `mode=all` 时生效.
...
拦截 Enter 后, 主线程会先做风险判断, 低风险静默放行, 高风险才弹窗。五类检测任一命中即视为高风险:
...
| @提及 | `@所有人`/`@all` (mode=all) 或任何@ (mode=any) 或自定义@词 | `at_trigger_mode`, `at_extra_keywords` |
...
**@检测模式**:
- `all` (默认): 仅 `@所有人`/`@all`/`@全体` + 用户自定义@词 (如 `@老板`) 触发. 普通 `@张三` 不弹.
- `any`: 任何含 `@` 的消息都弹.
- `off`: 关闭@检测.
...
**已知限制**: 微信 PC 版窗口标题恒为"微信", 群/联系人切换检测不到, 只能检测 app 级切换 (如 钉钉↔微信)。钉钉/飞书标题格式为 `群名 - 钉钉`, 可正常检测群切换。高危群检测在微信上仅能匹配恒定标题"微信", 实际不生效。
