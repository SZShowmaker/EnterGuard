...
- **第二阶段: 风险检测**, 高风险才弹窗, 低风险静默放行。四类检测, 任一命中即高风险:
  ...
  - 已知限制: 飞书(Chromium内核)和微信(Qt自绘)的会话列表/输入框均未暴露给UIA, 仅 app级切换 + 静默时段 可用; 高危群依赖真群名, 仅老版钉钉生效 (详见下表)
- 未来可扩展: 白名单等
...
**已知限制**:
- **微信 PC**: Qt 自绘界面, UIA 控件树只暴露顶部导航栏, 会话列表/聊天区/输入框均未暴露。会话名恒为"微信", 群切换检测不到, 输入框内容读不到。
- **飞书 PC**: Chromium 内核 (Electron 类), UIA 只能看到 Chromium 外壳 (`Chrome_RenderWidgetHostHWND`), 会话列表/群名/输入框都在 web 内容区里, UIA 读不到 DOM。会话名恒为"飞书", 群切换检测不到, 输入框内容读不到。
- **高危群检测**: 依赖真群名。新版钉钉伪群名 `钉钉#0` 不含真实群名关键词, 微信/飞书群名恒定, 此检测仅对老版钉钉 (能从标题切出真群名) 有效。

**各检测对不同应用的可用性**:

| 检测 | 钉钉(新版,Qt) | 钉钉(老版) | 飞书(Chromium) | 微信(Qt) |
|------|--------------|-----------|---------------|---------|
| 聊天对象变化 | ✅ 群级(伪指纹) | ✅ 群级(真群名) | ⚠️ 仅 app 级 | ⚠️ 仅 app 级 |
| 敏感词 | ✅ | ✅ | ❌ 读不到内容 | ❌ 读不到内容 |
| 高危群 | ❌ 伪群名无关键词 | ✅ | ❌ 群名恒定 | ❌ 群名恒定 |
| 静默时段 | ✅ | ✅ | ✅ | ✅ |

> 注: 钉钉新版用会话列表选中项索引指纹 (`钉钉#0`) 实现了群级切换检测, 是三个应用中唯一能在新版上做到群级的。飞书(Chromium)和微信(Qt)的会话列表均未暴露给 UIA, 群级检测不可用。
