...
- **第二阶段: 风险检测**, 高风险才弹窗, 低风险静默放行。四类检测, 任一命中即高风险:
  ...
  - 已知限制: 微信PC读不到会话列表选中状态, 群切换检测不到, 只能检测app级切换; 高危群检测依赖会话名, 微信上不生效
- 未来可扩展: 白名单等
...
**已知限制**:
- 微信 PC 版窗口标题恒为"微信", UIA 也读不到会话列表选中状态, 群/联系人切换检测不到, 只能检测 app 级切换 (如 钉钉↔微信)。
- 高危群检测依赖会话名, 微信上会话名恒为"微信", 不生效; 新版钉钉伪群名 `钉钉#0` 不含真实群名关键词, 也不生效。高危群目前仅对老版钉钉/飞书 (能切出真群名) 有效。
